...
(defn- -sync-database! [driver database]
  (let [active-tables      (i/active-tables driver database)
...
(defn sync-database!
  ...
  (binding [i/*disable-qp-logging* true
    ...
    (i/sync-in-context driver database (partial -sync-database-with-tracking! driver database))))
...
(defn- sync-metabase-metadata-table!
  ...
  [driver database]
  {:keys [table-rows-seq active-tables]}
  (doseq [{table-name :name} (i/active-tables driver database)]
    (when (= (s/lower-case table-name) "_metabase_metadata")
      (doseq [{:keys [keypath value]} (i/table-rows-seq driver database table-name)]
...
(defn sync-table!
  ...
  [driver table]
  (binding [i/*disable-qp-logging* true]
    (i/sync-in-context driver @(:db table) (fn []
...
(defn- sync-table-active-fields-and-pks!
  ...
  (let [active-column-names->type  (i/active-column-names->type driver table)
    ...
    (u/try-apply update-table-pks! table (i/table-pks driver table))))
...
(defn- sync-table-fks! [driver table]
  (when (contains? (i/features driver) :foreign-keys)
    (let [fks (i/table-fks driver table)]
      (assert (and (set? fks)
                   (every? map? fks)
...
(defn- sync-field!
  ...
  [driver field]
  {:pre [driver field]}
  (loop [field field, [f & more] [(partial i/driver-specific-sync-field! driver)
                                  set-field-display-name-if-needed!
...
(defn- mark-url-field!
  ...
  (when (and (not (:special_type field))
    ...
    (when-let [percent-urls (i/field-percent-urls driver field)]
...
(defn- mark-no-preview-display-field!
  ...
  (when (and (:preview_display field)
             (contains? #{:CharField :TextField} (:base_type field)))
    (let [avg-len (i/field-avg-length driver field)]
...
(defn- mark-json-field!
  ...
  (when (and (not (:special_type field))
             (contains? #{:CharField :TextField} (:base_type field))
             (values-are-valid-json? (->> (i/field-values-lazy-seq driver field)
...
(defn- sync-field-nested-fields! [driver field]
  (when (and (= (:base_type field) :DictionaryField)
             (contains? (i/features driver) :nested-fields))
    ;; fetch existing nested fields
    (let [nested-field-name->type        (i/active-nested-field-name->type driver field)
          existing-nested-field-name->id (sel :many :field->id [Field :name], :table_id (:table_id field), :active true, :parent_id (:id field))]
      ...
      (doseq [[nested-field-name nested-field-id] existing-nested-field-name->id]
        (when-not (contains? (set (map keyword (keys nested-field-name->type)))
                             (keyword nested-field-name))
      ...
      (doseq [[nested-field-name nested-field-type] nested-field-name->type]
        (when-not (contains? (set (map keyword (keys existing-nested-field-name->id)))
                             (keyword nested-field-name))
